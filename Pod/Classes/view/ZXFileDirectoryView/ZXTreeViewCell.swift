...
class ZXTreeViewCell: UITableViewCell {
    ...
    var open:Bool = false {
        didSet {
            ...
            self.iconButton.setImage(UIImage.zx_imageNamed(imageName), forState: UIControlState.Normal)
        }
    }
    
    private lazy var iconButton: UIButton = {
        let new = UIButton()
        new.setImage(UIImage.zx_imageNamed("dir"), forState: .Normal)
        return new
    }()
    ...
    func configure(item:ZXTreeItem) {
        ...
        if item.isDirectory {
            self.iconButton.setImage(UIImage.zx_imageNamed("dir"), forState: .Normal)
        }else {
            self.iconButton.setImage(UIImage.zx_imageNamed("code"), forState: .Normal)
        }
